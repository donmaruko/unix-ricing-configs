...
std::vector<ChessBoard::CharacterBoard> ChessBoard::findAllQueenPlacements() {
    ...
    std::vector<ChessBoard::CharacterBoard> allBoards;     // Completed solutions
...
void ChessBoard::queenHelper(const int& col,
    ...
    std::vector<ChessBoard::CharacterBoard>& allBoards)
{
    // base case
    if (placedQueens.size() == BOARD_LENGTH) {
        /* Convert current ChessPiece board → CharacterBoard */
        ChessBoard::CharacterBoard snapshot(BOARD_LENGTH, std::vector<char>(BOARD_LENGTH, '.'));
    ...
    for (int row = 0; row < BOARD_LENGTH; ++row) {
        ...
        for (Queen* other : placedQueens) {
            if (other->canMove(row, col, board)) {
                safe = false;
                break;
            }
        }
        ...
        /* Backtrack: remove queen & free memory */
...
namespace {
                    ...
                    const ChessBoard::CharacterBoard& B) 
    {
        ...
        cur = flipAcrossHorizontal(A);
        for (int r = 0; r < 4; ++r) {
            if (cur == B) return true;
            cur = rotate(cur);
        }

        cur = flipAcrossVertical(A);
        for (int r = 0; r < 4; ++r) {
            if (cur == B) return true;
            cur = rotate(cur);
        }
...
ChessBoard::groupSimilarBoards(const std::vector<ChessBoard::CharacterBoard>& boards)
{
    std::vector<std::vector<ChessBoard::CharacterBoard>> groups;

    for (const auto& bd : boards) {
        // Validate the board: ensure exactly one 'Q' per row and column
        std::vector<int> rowCounts(BOARD_LENGTH, 0);
        std::vector<int> colCounts(BOARD_LENGTH, 0);
        bool valid = true;
        ...
        // Group the board into equivalence classes
        bool placed = false;
        for (auto& g : groups) {
        ...
        if (!placed) {
            groups.push_back(std::vector<ChessBoard::CharacterBoard>{bd});
        }
    }
 ...
 */
template <typename T>
std::vector<std::vector<T>> Transform::rotate(const std::vector<std::vector<T>>& m) {
    const std::size_t n = m.size();
    if (!n) return {};
    std::vector<std::vector<T>> out(n, std::vector<T>(n));
    for (std::size_t i = 0; i < n; ++i)
        for (std::size_t j = 0; j < n; ++j)
            out[j][n-1-i] = m[i][j];          // (i,j) → (j,n-1-i)
    return out;
}
 ...
 */
template <typename T>
std::vector<std::vector<T>> Transform::flipAcrossVertical(const std::vector<std::vector<T>>& m) {
    const std::size_t n = m.size();
    if (!n) return {};
    auto out = m;                             // copy
    for (std::size_t i = 0; i < n; ++i)
 ...
 */
template <typename T>
std::vector<std::vector<T>> Transform::flipAcrossHorizontal(const std::vector<std::vector<T>>& m) {
    const std::size_t n = m.size();
    if (!n) return {};
    auto out = m;                             // copy
...
int main() {
    ...
    if (solutions.size() != 92) {
        std::cerr << "Error: Expected 92 unique solutions, but got " << solutions.size() << '\n';
    } else {
        std::cout << "Correct number of solutions found (92).\n";
    }

    // Print the first solution for verification
    ...
    /* ------------ Grouping check ------------ */
    auto classes = ChessBoard::groupSimilarBoards(solutions);
    std::cout << "Distinct equivalence classes: " << classes.size() << '\n';

    // Check if the number of equivalence classes is correct
    if (classes.size() != 12) {
        std::cerr << "Error: Expected 12 distinct equivalence classes, but got " << classes.size() << '\n';
    } else {
        std::cout << "Correct number of equivalence classes found (12).\n";
    }

    // Print the size of each equivalence class
    std::cout << "Class sizes: ";
    for (const auto& c : classes) std::cout << c.size() << ' ';
    std::cout << '\n';
    ...
    for (const auto& board : solutions) {
        ...
        if (!valid) {
            all_valid = false;
            std::cerr << "Error: Invalid board detected:\n";
    ...
    if (all_valid) {
        std::cout << "All solutions are valid.\n";
    } else {
        std::cerr << "Some solutions are invalid.\n";
    }
